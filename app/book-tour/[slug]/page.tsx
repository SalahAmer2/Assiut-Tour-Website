...
export default function BookTour({ params }: { params: { slug: string } }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const res = await fetch("/api/create-checkout-session", {
      ...
      body: JSON.stringify({
        tour: params.slug, // which tour they’re booking
        formData,
      }),
    });
  ...
  return (
    <main className="flex flex-col items-center py-12 px-6">
      <h1 className="text-3xl font-bold mb-8">Book {params.slug.replace("-", " ")}</h1>
      <form
      ...
      >
        ...
        <input type="email" name="email" placeholder="Email Address" onChange={handleChange} required className="border p-2 w-full"/>
